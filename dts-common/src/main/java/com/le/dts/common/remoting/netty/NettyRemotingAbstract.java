...
import com.le.dts.common.domain.remoting.SemaphoreReleaseOnlyOnce;
import com.le.dts.common.domain.remoting.ServiceThread;
import com.le.dts.common.exception.RemotingTooMuchRequestException;
import com.le.dts.common.helper.RemotingHelper;
import com.le.dts.common.remoting.RPCHook;
...
import com.le.dts.common.remoting.InvokeCallback;
import com.le.dts.common.remoting.RPCHook;
import com.le.dts.common.remoting.protocol.RemotingCommand;
import com.le.dts.common.remoting.protocol.RemotingSysResponseCode;
    ...
    class NettyEventExecuter extends ServiceThread {
        ...
        public void putNettyEvent(final NettyEvent event) {
            if (this.eventQueue.size() <= MaxSize) {
                ...
                plog.warn("event queue size[{" + this.eventQueue.size() + "}] enough, so drop this event {" + event.toString() + "}");
            }
        }
            ...
            try {
                channel.writeAndFlush(request).addListener(new ChannelFutureListener() {
                    ...
                    public void operationComplete(ChannelFuture f) throws Exception {
                        ...
                        plog.warn("send a request command to channel <{" + RemotingHelper.parseChannelRemoteAddr(channel) + "}> failed.");
                        plog.warn(request.toString());
                    }
                });
            }
